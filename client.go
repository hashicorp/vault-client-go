...
func applyTLSOptions(to *tls.Config, from TLSOptions) error {
	...
	switch {
	case from.ClientCertFile != "" && from.ClientCertKey != "":
	...
	case from.ClientCertFile != "":
		return fmt.Errorf("client certificate file %q is specified but certificate key is missing", from.ClientCertFile)

	case from.ClientCertKey != "":
		return fmt.Errorf("client certificate key %q is specified but certificate file is missing", from.ClientCertKey)
	}
