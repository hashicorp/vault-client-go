...
// applyTLS applies the given TLS options to the given *tls.Config object
func applyTLS(to *tls.Config, options TLSOptions) error {
	switch {
	...
	if len(options.CAFile) != 0 || len(options.CACertificate) != 0 || len(options.CAPath) != 0 {
		...
		if err := rootcerts.applyTLS(to, rootCertificateConfig); err != nil {
			return fmt.Errorf(
